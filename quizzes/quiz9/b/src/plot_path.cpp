            ...
            if (imageOK)
            {
                ...
                if (pathArrayBuffer_.received)
                {
                    ...
                    for (unsigned int i = 0; i < pathArrayBuffer_.poses.poses.size(); i++)
                    {
                        ...
                        double x_position = pose.position.x - pathArrayBuffer_.vehPose.position.x;
                        double y_position = pose.position.y - pathArrayBuffer_.vehPose.position.y;
                        ROS_INFO_STREAM("Localized position [x,y]: [ " << x_position << " ],[ " << y_position << " ]");
                        double xD = (x_position / pathArrayBuffer_.resolution) + offset;
                        double yD = (-y_position / pathArrayBuffer_.resolution) + offset;
