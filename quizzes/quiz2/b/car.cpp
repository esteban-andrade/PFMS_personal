...
Car::Car(std::string make, std::string model, double height, double width,
         double horsePower, double dragCoefficient, double weight) : make_(make), model_(model), dragCoefficient_(dragCoefficient),
...
bool Car::setStatus(bool status)
{
  status_ = status;
  //return status_;
}
...
void Car::timeTracker(void)
{
 timer_tracker_ = std::chrono::steady_clock::now();
}

// std::chrono::duration<double>Car::timer()
// {   
//     // track time since Camera starting polling data.
//     std::chrono::time_point timer =std::chrono::steady_clock::now();
