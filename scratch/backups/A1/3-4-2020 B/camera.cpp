...
void Camera::timeTracker(void)
{
    ...
    start_config_time = std::chrono::steady_clock::now();
}

std::chrono::duration<double> Camera::getTimeFirstQuery()
{
    std::chrono::duration<double> time_spam = std::chrono::duration_cast<std::chrono::duration<double>>(std::chrono::steady_clock::now() - start_config_time);
    return time_spam;
}
...
int Camera::getElapsedTime()
{
    ...
    elapsed_time_= std::chrono::duration_cast<std::chrono::seconds>(end_time - timer).count();
