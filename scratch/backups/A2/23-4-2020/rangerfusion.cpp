    ...
    for (auto it : rangers_)
    {
        ...
        if (sensing_method == CONE)
        {
            ...
            for (auto d : data)
            {
                ...
                double angle;
                angle = (double)-offset - ((double)field_of_view / 2.0);
                sonar.endA.x = d * cos(degreesToRadians(angle));
                sonar.endA.y = d * sin(degreesToRadians(angle));

                angle = (double)-offset - ((double)field_of_view / 2.0);
                sonar.endB.x = d * cos(degreesToRadians(angle));
                sonar.endB.y = d * sin(degreesToRadians(angle));
    ...
    for (auto it : cells_)
    {
        ...
        for (auto sonar : sonars)
        {
            if (sonar.valid)
            {
                ...
                bool cell_inside_sonar = isPointInTriangle(sonar.origin, sonar.endA, sonar.endA, cell.centre);
                if (intersections > 0)
                {
                    updateCellState(*it, FREE);
                }
...
{
    if (line.p1.x == point.x)
    {
        return true;
    }
    else if (line.p1.y == point.y)
    {
        return true;
    }
    else if (line.p2.x == point.x)
    {
        return true;
    }
    else if (line.p2.y == point.y)
    {
        return true;
    }
    else
