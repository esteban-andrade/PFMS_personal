...
void Scanner::predictTarget(const std::shared_ptr<Simulator> &sim)
{
    ...
    double vx = (second_x - original_x) / 0.1 + time_def;
    double vy = (second_y - original_y) / 0.1 + time_def;
    velocity_vector_.push_back(std::make_pair(vx, vy));
    predicted_x = second_x + velocity_x * (time_def + 0.1);
    predicted_y = second_y + velocity_y * (time_def + 0.1);
