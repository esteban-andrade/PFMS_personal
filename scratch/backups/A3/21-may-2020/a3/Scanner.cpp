...
{
    std::vector<RangeBearingStamped> raw_scan = sim->rangeBearingToBogiesFromFriendly();
...
void Scanner::determineTarget(const std::shared_ptr<Simulator> &sim)
{
    ...
    std::vector<RangeBearingStamped> bogies = sim->rangeBearingToBogiesFromFriendly();
    ...
    double angle = min_distance_relative_friendly.bearing;
    bearing_angle_ = angle + friendly_orientation;
    double final_x;
    double final_y;

    final_x = x_original + distance * cos(bearing_angle_);
    final_y = y_original + distance * sin(bearing_angle_);
...
void Scanner::predictTarget(const std::shared_ptr<Simulator> &sim)
{
    ...
    velocity_vector_.push_back(std::make_pair(velocity_x, velocity_y));
    predicted_x = second_x + velocity_x * (time_def + 0.5);
    predicted_y = second_y + velocity_y * (time_def + 0.5);
