                          ...
                          a4_setup::RequestGoal::Response &res)
{
  ...
  if (!image.empty())
  {
    ...
    req.pose.x -= x;
    req.pose.y -= y;
    ROS_INFO_STREAM("Local position [x,y] " << req.pose.x << " , " << req.pose.y);
    auto x_target = (req.pose.x / resolution_) + offset; //for image
    auto y_target = (-req.pose.y / resolution_) + offset;
    //use different ones
    cv::Point org(100, 100);
    cv::Point dest(static_cast<int>(x_target), static_cast<int>(y_target));

    imageProcessing_.setImage(image);
    res.ack = imageProcessing_.checkConnectivity(org, dest);
  }
