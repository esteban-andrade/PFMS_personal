...
void PfmsSample::odomCallback(const nav_msgs::OdometryConstPtr &msg)
{
  /**
     ...
     */
  //ROS_INFO("f", msg->pose.pose.orientation.x);
  // ROS_INFO("f", msg->pose.pose.orientation.y);
...
void PfmsSample::laserCallback(const sensor_msgs::LaserScanConstPtr &msg)
{
  ...
  ROS_INFO_STREAM("Nearest Obstacle in laser : " << x << " , y ::" << y);
  pose_mtx_.lock();
  nearestPoint_.x = x;
  nearestPoint_.y = y;
  pose_mtx_.unlock();
...
void PfmsSample::seperateThread()
{
  ...
  while (ros::ok())
  {
    ...
    if (pose_mtx_.try_lock_for(std::chrono::milliseconds(5)))
    { //This tries to mutex lock for 5ms and then gives up
      pose = pose_;
      pose_mtx_.unlock();
      have_pose = true;
    }

    if (point_mtx_.try_lock_for(std::chrono::milliseconds(5)))
    { //This tries to mutex lock for 5ms and then gives up
      pt = nearestPoint_;
      point_mtx_.unlock();
      have_point = true;
    ...
    if (have_pose && have_point)
    { //If we have both nearest point and pose we can compute it in global.
